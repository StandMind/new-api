...
export default function GroupRatioSettings(props) {
  ...
  const groupNames = useMemo(() => {
    ...
    const modelRatioMap = parseJSONSafe(inputs.GroupModelRatio, {});
    return [
      ...new Set([...Object.keys(ratioMap), ...Object.keys(modelRatioMap)]),
    ];
  }, [inputs.GroupRatio, inputs.GroupModelRatio]);
    ...
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      ...
      <Form.Section text={t('分组特殊倍率')}>
        ...
        <Form.TextArea
          label={t('模型专属分组倍率')}
          extraText={t(
            '嵌套 JSON：计费分组 → 精确模型或末尾星号前缀 → 倍率。精确模型优先，其次采用最长前缀。',
          )}
          ...
          onChange={(value) =>
            setInputs((prev) => ({ ...prev, GroupModelRatio: value }))
          }
        />
      </Form.Section>
      ...
      <Form.Section text={t('分组JSON设置')}>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('模型专属分组倍率')}
              placeholder={t('为一个嵌套 JSON 文本')}
              extraText={t(
                '计费分组映射到模型倍率；支持精确模型和末尾星号前缀，例如：{"vip":{"gpt-4o":0.8,"gpt-4*":0.9}}',
              )}
              field={'GroupModelRatio'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
