...
export function showError(error) {
  ...
  if (error?.name === 'AxiosError' && error.response?.status === 401) {
    Toast.error(extractApiErrorMessage(error), showErrorOptions);
...
export const calculateModelPrice = ({
  ...
  const recordGroupRatio =
    record?.group_ratio &&
    typeof record.group_ratio === 'object' &&
    !Array.isArray(record.group_ratio)
      ? record.group_ratio
      : groupRatio || {};
  const readGroupRatio = (group) => {
    const ratio = recordGroupRatio[group];
  ...
  if (!selectedGroup || selectedGroup === 'all') {
    ...
    usedGroupRatio = readGroupRatio(selectedGroup) ?? 1;
  }
...
export const formatDynamicPriceSummary = (billingExpr, t, groupRatio = 1) => {
  ...
  const gr =
    typeof groupRatio === 'number' && Number.isFinite(groupRatio)
      ? groupRatio
      : 1;
