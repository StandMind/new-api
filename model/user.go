...
type User struct {
	...
	invitationReward invitationRegistrationReward `json:"-" gorm:"-:all"`
}
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			reward, err := applyInvitationRegistrationTx(tx, user, inviterId)
			if err != nil {
				return err
			}
			user.invitationReward = reward
			return nil
		})
	}); err != nil {
		return err
...
func (user *User) finishInsert(inviterId int) {
	...
	if user.invitationReward.InviteeQuota > 0 {
		RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("使用邀请码赠送 %s", logger.LogQuota(user.invitationReward.InviteeQuota)))
	}
	if user.invitationReward.InviterQuota > 0 {
		RecordLog(user.invitationReward.InviterId, LogTypeSystem, fmt.Sprintf("邀请用户赠送 %s", logger.LogQuota(user.invitationReward.InviterQuota)))
	}
}
