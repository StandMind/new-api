...
和 1300 条精确模型级分组倍率。加上早期已迁移的 28 个同名模型，实际有 OpenLux
来源的公开模型总数为 315；无同名来源的 `gpt-5.4-2026-03-05` 已隐藏。

原 `default` 不能直接移除，否则旧 Key 和新用户默认 Key 会失效。生产为这 28 个早期
模型逐个选择售价最低且已验证可用的 OpenLux 分组，再按来源汇总为 6 个
`OpenLux-Default/*` 兼容渠道和 28 条 `default` Ability。`default` 的精确模型倍率复制
获选分组已经包含 30% 利润的售价，因此不会重复乘利润。生产当前共有 80 个启用的
OpenLux 渠道和 1518 条启用 Ability：早期单组渠道 16/190、扩展渠道 58/1300、
`default` 兼容渠道 6/28。加上 `default`，用户可选分组总数为 59；原系统内部
`group_1` 不在用户可选分组清单中。
...
结算，普通音频、WSS 和文本路径均有回归测试。生产最小 TTS 验证使用
`Azure-Gpt-2 -> gpt-4o-mini-tts`，返回 `audio/mpeg`，消费日志为
`0.3 × 0.133835 × 500000 = 20075.25`，截断后 quota 为 `20075`。上游返回 429、
模型不存在、渠道不可用或鉴权失败的组合只记为外部可用性跳过，不视为本地路由成功。

逐项价格审计要求 1300 个扩展组合均为 OpenLux 同组合价格的 1.3 倍；`default` 的
28 个组合通过复制已审计的获选分组售价保持同样利润。完整上游 Key 只存在于 OpenLux
和生产 Channel 密钥字段，不出现在 manifest、报告、日志、文档或 Git。3 个历史云雾
渠道仅保留为回滚记录，状态和 Ability 均为禁用，不再参与任何路由；所有临时系统访问
令牌和冒烟 Key 在验证结束后清理。
