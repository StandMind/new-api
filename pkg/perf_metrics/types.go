...
func (b *atomicBucket) add(sample Sample) {
	...
	if sample.Success {
		...
		if sample.LatencyMs > 0 {
			b.successLatencyMs.Add(sample.LatencyMs)
		}
	}
	if sample.LatencyMs > 0 {
...
func (b *atomicBucket) snapshot() counters {
	return counters{
		...
		successLatencyMs: b.successLatencyMs.Load(),
...
func (b *atomicBucket) drain() counters {
	return counters{
		...
		successLatencyMs: b.successLatencyMs.Swap(0),
...
func (b *atomicBucket) addCounters(c counters) {
	...
	if c.successLatencyMs != 0 {
		b.successLatencyMs.Add(c.successLatencyMs)
	}
	if c.ttftSumMs != 0 {
