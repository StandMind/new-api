...
func PasskeyRegisterBegin(c *gin.Context) {
	if !system_setting.GetPasskeySettings().Enabled {
		common.ApiErrorI18n(c, i18n.MsgPasskeyDisabled)
		return
	}

	user, err := getSessionUser(c)
	if err != nil {
		common.ApiErrorStatus(c, http.StatusUnauthorized, err)
		return
	}
...
func PasskeyVerifyFinish(c *gin.Context) {
	...
	if err := session.Save(); err != nil {
		common.SysError("failed to save passkey verification state: " + err.Error())
...
func getSessionUser(c *gin.Context) (*model.User, error) {
	...
	if idRaw == nil {
		return nil, common.NewPublicError(i18n.MsgUnauthorized, nil)
	}
	id, ok := idRaw.(int)
	if !ok {
		return nil, common.NewPublicError(i18n.MsgPasskeySessionInvalid, nil)
	}
	user := &model.User{Id: id}
	if err := user.FillUserById(); err != nil {
		return nil, err
	...
	if user.Status != common.UserStatusEnabled {
		return nil, common.NewPublicError(i18n.MsgUserDisabled, nil)
	}
	return user, nil
}
...
func requireSecureVerificationMethod(c *gin.Context, method string) bool {
	...
	if !ok || time.Now().Unix()-verifiedAt >= SecureVerificationTimeout {
		...
		session.Delete(secureVerificationMethodSessionKey)
		_ = session.Save()
		common.ApiErrorI18n(c, i18n.MsgPasskeySecurityVerificationRequired)
		return false
	}

	if verifiedMethod, ok := session.Get(secureVerificationMethodSessionKey).(string); !ok || verifiedMethod != method {
		common.ApiErrorI18n(c, i18n.MsgPasskeyMatchingVerificationRequired)
		return false
	}
