...
func SubscriptionRequestCreemPay(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.TranslateMessage(c, i18n.MsgInvalidRequestBody)})
		return
	}
	...
	if creemProductId == "" {
		if setting.CreemTestMode {
			common.ApiErrorI18n(c, i18n.MsgPaymentProductNotConfig)
			return
		}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem subscription customer email error user_id=%d error=%q", userId, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": common.TranslateMessage(c, i18n.MsgPaymentCustomerEmailRequired)})
		return
	}
