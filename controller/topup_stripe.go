...
func (*StripeAdaptor) RequestAmount(c *gin.Context, req *StripePayRequest) {
	if req.Amount < getStripeMinTopup() {
		common.ApiErrorDataI18n(c, i18n.MsgTopupAmountMinimum, map[string]any{"Min": getStripeMinTopup()})
		return
	}
	id := c.GetInt("id")
	group, err := model.GetUserLevel(id, true)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("failed to load user level for Stripe user_id=%d error=%q", id, err.Error()))
...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if req.Amount < getStripeMinTopup() {
		c.JSON(http.StatusOK, gin.H{"message": i18n.T(c, i18n.MsgTopupAmountMinimum, map[string]any{"Min": getStripeMinTopup()}), "data": 10})
		return
	}
	if req.Amount > 10000 {
		c.JSON(http.StatusOK, gin.H{"message": i18n.T(c, i18n.MsgTopupAmountMaximum, map[string]any{"Max": 10000}), "data": 10})
		return
	}

	if req.SuccessURL != "" && common.ValidateRedirectURL(req.SuccessURL) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": i18n.T(c, i18n.MsgPaymentCallbackError), "data": ""})
		return
	}
