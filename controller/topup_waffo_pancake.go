...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	if req.Amount < int64(setting.WaffoPancakeMinTopUp) {
		common.ApiErrorDataI18n(c, i18n.MsgTopupAmountMinimum, map[string]any{"Min": setting.WaffoPancakeMinTopUp})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("failed to load user level for Waffo Pancake user_id=%d error=%q", id, err.Error()))
...
func SaveWaffoPancake(c *gin.Context) {
	...
	if err := service.SaveWaffoPancakeConfig(
		...
		common.ApiErrorDataI18n(c, i18n.MsgUpdateFailed)
		return
	}
...
func CreateWaffoPancakePair(c *gin.Context) {
	...
	if err != nil {
		...
		data := gin.H{"error": i18n.T(c, i18n.MsgPaymentCreateFailed)}
		if orphan {
...
func CreateWaffoPancakeSubscriptionProduct(c *gin.Context) {
	...
	if strings.TrimSpace(req.Name) == "" {
		common.ApiErrorDataI18n(c, i18n.MsgSubscriptionTitleEmpty)
		return
	}
...
func RequestWaffoPancakePay(c *gin.Context) {
	...
	if err != nil || user == nil {
		common.ApiErrorDataI18n(c, i18n.MsgUserNotExists)
		return
	}

	group, err := model.GetUserLevel(id, true)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("failed to load user level for Waffo Pancake checkout user_id=%d error=%q", id, err.Error()))
		common.ApiErrorDataI18n(c, i18n.MsgOperationFailed)
		return
	}
